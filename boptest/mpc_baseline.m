%% mpc_baseline.m: The MPC that carries the model-plant mismatch (Section 4.2)
%
%  The QP predicts with the identified 2C model alone. There is no disturbance
%  estimation and therefore no integral action, so the mismatch between the
%  model and the building shows up as a steady-state error. This is the
%  controller the RL of Chapter 5 and the BO of Chapter 6 are applied on top of.
%
%  It differs from ekf_mpc.m only in that tracking_mpc_step is called with
%  use_bias = false. The 15-state KF still runs, but its bias entries are held
%  at zero, so d̂ = 0 enters both Y_free and the (x̄, ū) target.
%
%  BEFORE RUNNING:
%    1. startup
...
%
%  Saves  models/mpc_baseline[<tag>]_simdata.mat
...
if ~exist('models/tracking_mpc_params.mat', 'file')
    error('[mpc_baseline] Run mpc/tracking_mpc_params_build first.');
end
...
USE_BIAS = false;   % no offset-free correction

fprintf('[mpc_baseline] ||Lkf||_F=%.4f   nx=%d   nz=%d   Np=%d   Ts=%.0fs\n', ...
        norm(p.Lkf,'fro'), p.nx, p.nz, p.N_p, Ts);
fprintf('[mpc_baseline] use_bias=%d  →  no integral action, no offset correction\n', USE_BIAS);
...
if USE_NOISY_FCAST
    ...
    fprintf('[mpc_baseline] noisy fcast: per-step refetch (T_unc=%s, S_unc=%s)\n', ...
...
%% 6. Initialise the KF state (the bias entries stay zero)
Tz_all = zeros(nZ_boptest,1);
for i = 1:nZ_boptest, Tz_all(i) = res.(SIG_TZ{i}); end
Tair_init = Tz_all(1:nZ);
z_aug     = [Tair_init; Tair_init + 1.5; zeros(p.nd_bias, 1)];   % [x; 0]
u_prev    = zeros(nU,1);
...
fprintf('MPC running [tracking, no bias correction]: %d zones | %g day(s) | Np=%d (%.0fh)\n\n', ...
...
for i = 1:n_steps
    ...
    [v_cmd, u_apply, z_aug, ~, innov_kf] = tracking_mpc_step( ...
        Tz_meas, dvec, lb_horizon, z_aug, u_prev, USE_BIAS);

    % 6. One-step model prediction, logged as the implied setpoint
    x_hat        = z_aug(1:nx);
    Tamb_now     = dvec(1);
    d_now        = dvec(1:p.nD);
    T_model_next = p.C_ctrl_m * (p.A * x_hat + p.Bu * u_apply + p.Bd * d_now);
    ...
    if mod(i,16)==0
        ...
        fprintf(']  v=['); fprintf('%.2f ', v_cmd'); fprintf(']\n');
    end
end
...
metrics = kpi_report('mpc_baseline', kpi, log_Tz, log_Qhea, log_ref, log_ub, ...
                     ZONE_NAMES, Ts, p.occ_sp_threshold);
comfort_plot('mpc_baseline', log_Tz, log_ref, log_ub, log_Tamb, kpi, ZONE_NAMES, Ts);
...
sim.Tamb     = log_Tamb;
sim.innov    = log_innov;
sim.use_bias = USE_BIAS;
sim.kpi      = kpi;
sim.metrics  = metrics;
sim.BOPTEST_TUNC = BOPTEST_TUNC;
sim.BOPTEST_SUNC = BOPTEST_SUNC;

out_fname = sprintf('mpc_baseline%s_simdata.mat', BOPTEST_OUT_TAG);
save(fullfile('models', out_fname), 'ss_model', 'sim', ...
     'kpi', 'metrics', 'Ts', 'BOPTEST_TUNC', 'BOPTEST_SUNC', 'start_time');
fprintf('[mpc_baseline] Saved  models/%s\n', out_fname);
